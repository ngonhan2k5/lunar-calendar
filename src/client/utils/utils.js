...
const Utils = (function(){
    ...
    */
    ...
    function getToday(){    
        return {sday:today.getDate(), smonth:today.getMonth()+1, syear:today.getFullYear() }
    ...
    */
    ...
    /**
     * Populate memo for a solar month
     * @param {*} yy 
     * @param {*} mm 
     */
    var populate = function (memos, yy, mm){
        var currentMonth = Utils.getMonth(mm, yy);
        var uniqueMap = {}
        return memos.map(
                (item) => {
                let l_when = item.l_when
                for(var i=0; i<currentMonth.length; i++){
                    let cd = currentMonth[i]
                    if (l_when.day == cd.day ){
                        if (l_when.month == cd.month || l_when.month == 0){
                            if (l_when.year == cd.year || l_when.year == 0){
                                return Object.assign({}, item, {
                                    href:`/${yy}/${mm}/${i+1}`, 
                                    sday:i+1, col1:`${yy}/${mm}/${i+1}`, 
                                    col2:`${cd.day}.${cd.month}`
                                })
                            }
                        }
                    }
            ...
            (item) => {
                if (typeof(item) == 'undefined')
                    return false
                else if (item.col1 in uniqueMap){
                    // uniqueMap[item.col1].title += ', '+ item.title
                    uniqueMap[item.col1].titleAndIds.push({_id:item._id, title:item.title})
                    return false
                }else{
                    item.titleAndIds = [{_id:item._id, title:item.title}]
                    
                    uniqueMap[item.col1] = item
                    return true;
                }
    ...
    var getMemo = function(memos, sday){
        let ret = memos.filter((item)=> item.sday == sday )
        return ret.length == 0? false: ret
        // for(var i = 0; i< memos.length; i++){
        //     if (sday == memos[i].sday)
        //         return true;
        // }
        // return false;
    }
